...
public class DateUtil {
    ...
    public static Date getBeforeYear(Date date, Integer year) {
        LocalDateTime localDate = LocalDateTime.ofInstant(date.toInstant(), ZoneId.systemDefault());
        localDate = localDate.plusYears(year * -1);
        return Date.from(localDate.atZone(ZoneId.systemDefault()).toInstant());
    }
...
//                + seconds + " seconds ";
    ...
    public static long date2TimeStamp(Date date,String formats){
        if (DataUtil.isEmpty(formats))
            formats = "yyyy-MM-dd HH:mm:ss";
        try {
...
//        System.out.println(strToDate("2019-12-15 16:48:49"));.
        ...
        //当前时间时间戳
        long tradeTimeStamp = date2TimeStamp(new Date(), null);

        System.out.println(tradeTimeStamp);
        //  System.out.println(TimeStamp2Date(String.valueOf(getMonthLastDay()),null));
